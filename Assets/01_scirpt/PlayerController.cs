...
public class PlayerController : MonoBehaviour
{
    ...
    private float limitDeathY; //플레이어가 사망하는 최소 y 값
    ...
    private void Awake()
    {
        ...
        // 현재 플레이어의 y위치에서 y/2 크기를 뺀 만큼이 사망하는 y 위치
    ...
    private void Update()
    {
        ...
        if(Input.GetMouseButtonDown(0))
        {
            ...
            // 현재 이동방향이 Vector3.right(1, 0, 0)이면 이동방향을 Vector.forward(0, 0, 1)으로 설정
            Vector3 direction = movement.MoveDirection == Vector3.forward ? Vector3.right : Vector3.forward;
            movement.MoveTo(direction);
            gameController.IncreaseScore();
        }
        // 플레이어의 y 위치가 limitDeathY보다 작으면 사망처리
