...
public class GameController : MonoBehaviour
{
    [Header("GameStart UI")]
    [SerializeField]
    private FadeEffect[] fadeGameStart; // 게임시작시 FadeIn 되어야 하는 UI들
    [SerializeField]
    private GameObject panelGameStart; // 게임시작시 Panel 오브젝트
    [SerializeField]
    private TextMeshProUGUI textGameStartBestScore; //게임시작시 최고 점수 출력용 TextUI

    [Header("InGame UI")]
    [SerializeField]
    private TextMeshProUGUI textInGameScore; // 게임에서 획득한 점수 출력 Text UI

    [Header("GameOver UI")]
    [SerializeField]
    private GameObject panelGameOver; //게임오버 Panel
    [SerializeField]
    private TextMeshProUGUI textGameOverScore; //게임오버 되었을 때 게임에서 획득한 점수 출력용 Text UIi
    [SerializeField]
    private TextMeshProUGUI textGameOverBestScore; //게임오버 되었을 때 최고 점수 출력용 Text UI
    [SerializeField]
    private float timeStopTime; //시간배율(Time.timeScale)이 0이 될 때까지 소요 시간
    ...
    {
        // score 만큼 현재 점수(currentScore) 증가ㅏ
        currentScore += score;

        // 변경된 점수(CurrentScore) 정보를 Text UI에갱신
    ...
    public void GameOver()
    {
        ...
        // 저장되어 있는 최고점수 불러오기
        int bestScore = PlayerPrefs.GetInt("BestScore");
        // 현재 스테이지에서 획득한 점수가 최고저ㅓㅁ수보다 높으면 최고점수 갱신
        if(currentScore > bestScore)
        {
            //currentScore 점수를 최고점수로 등록
            PlayerPrefs.SetInt("BestScore", currentScore);
            //currentScore 점수를 최고점수 TextUI 출력
            textGameOverBestScore.text = currentScore.ToString();
        }
        else
        {
            // 기존의 bestScore 점수를 최고점수 Text UI에 출력
